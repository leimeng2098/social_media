...
const route = useRoute() // 用來取得網址列的參數
const router = useRouter() // 用來做頁面跳轉

const postId = route.params.id // 從網址抓取 :id
const post = ref(null)
const comments = ref([])
...
const handleComment = async () => {
  ...
  try {
    ...
    fetchComments() // 重新拉取留言列表
    fetchPost() // 重新拉取貼文 (更新留言數量統計)
  } catch (error) {
